    ...
    <div className="ml-10 grid grid-cols-1 lg:grid-cols-3">
      {Object.values(ROOMS_DATA_DESKTOP).map((room, index) => {
        ...
        return (
          <div
            key={index}
            className={`relative group overflow-hidden rounded-lg h-[calc(100% / 1.8)]`}
            style={roomStyle}
          >
            <img
              src={baseImagePath(room.image)}
              alt={room.title}
              className="object-contain transition-opacity duration-300 group-hover:opacity-30 relative z-[1]"
            />
            <img
              src={baseImagePath(room.hoverImage)}
              alt={`${room.title} hover`}
              className="absolute z-[1] inset-0 object-contain opacity-0 group-hover:opacity-80 transition-opacity duration-300"
            />
            <img
              src={baseImagePath('desktop/Shadow.png')}
              alt={`${room.title} shadow`}
              className="absolute bottom-[0] object-contain -right-[1%]"
            />
